...
const EditTVChannel = () => {
 ...
 useEffect(() => {
  const fetchData = async () => {
   try {
    ...
    setFormData({
     ...channelData,
     category: channelData.category?._id || channelData.category
    });
   } catch (err) {
 ...
 if (initialLoading) return <div style={{color: '#fff', padding: '50px', textAlign: 'center'}}>Loading Channel Data...</div>;
 ...
 return (
  <div className="add-channel-page">
    ...
    <div className="form-container">
     ...
     <div className="form-column">
      ...
      {formData.streamType === 'Embed Code' ? (
       ...
       <>
        <div className="form-group row">
         ...
         <input type="text" name="server1Url" value={formData.server1Url} onChange={handleChange} required className="dark-input" />
        </div>

        <div className="form-group row">
         <label>Server 2 URL</label>
         <input type="text" name="server2Url" value={formData.server2Url} onChange={handleChange} className="dark-input" />
        </div>

        <div className="form-group row">
         <label>Server 3 URL</label>
         <input type="text" name="server3Url" value={formData.server3Url} onChange={handleChange} className="dark-input" />
        </div>
       </>
      )}
      ...
      <div className="form-group row">
       ...
       <div className="logo-upload-section">
        <div className="file-input-group">
         <input 
          type="text" 
          value={formData.logo ? 'Logo Loaded' : ''} 
          readOnly 
      ...
      <div className="form-group row">
       ...
       <input type="text" name="seoTitle" value={formData.seoTitle} onChange={handleChange} className="dark-input" />
      </div>
      <div className="form-group row">
       <label>Meta Description</label>
       <textarea name="metaDescription" value={formData.metaDescription} onChange={handleChange} className="dark-textarea" />
      </div>
      <div className="form-group row">
       <label>Keyword</label>
       <textarea name="keywords" value={formData.keywords} onChange={handleChange} className="dark-textarea" />
   ...
   <style dangerouslySetInnerHTML={{ __html: `
    ...
    .file-input-group .dark-input { border-radius: 4px 0 0 4px; flex: 1; }
