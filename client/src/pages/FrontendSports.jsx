...
const FrontendSports = () => {
 ...
 useEffect(() => {
  ...
  const fetchSports = async () => {
   ...
   try {
    ...
    console.log('[DEBUG] All sports count:', allSports.length);
    setSports(allSports);
   } catch (err) {
 ...
 const formatImageUrl = (item) => {
  if (!item) return null;
  // Prioritize poster for sports videos as per model
 ...
 return (
  <FrontendLayout isTransparent={true}>
     ...
     {loading ? (
      ...
      <div className="movies-grid">
       {sports.map((sport) => {
        ...
        return (
         <Link to={`/details/sports/${sport._id}`} key={sport._id} className="fe-movie-card-new">
          ...
          <div className="card-info-new">
           <div className="card-meta-top">
            <span className="age-badge">ATHLETIC</span>
            <span className="year-text">SPORTS</span>
           </div>
           <span className="genre-text-red">{categoryName}</span>
           <h3 className="movie-title-v">{sportTitle}</h3>
          </div>
         </Link>
        );
       })}
   ...
   <style dangerouslySetInnerHTML={{ __html: `
    ...
    .fe-movies-hero { position: absolute; top: 0; left: 0; width: 100%; height: 100%; min-height: 350px; background: url('https://images.unsplash.com/photo-1504450758481-7338eba7524a?w=1920&q=80') center/cover; background-attachment: scroll; display: flex; align-items: center; padding: 0 10%; color: #fff; z-index: 1; overflow: hidden; cursor: pointer; }
    ...
    .fe-premium-badge-v { position: absolute; top: 15px; left: 15px; display: flex; border: 1px solid #000; border-radius: 2px; overflow: hidden; background: #fff; line-height: 1; height: 18px; }
    ...
    @media (max-width: 768px) { 
     .fe-movies-hero { position: relative; height: auto; min-height: 220px; background-attachment: scroll; padding: 80px 20px 40px; background-image: url('https://images.unsplash.com/photo-1504450758481-7338eba7524a?w=800&q=80'); }
