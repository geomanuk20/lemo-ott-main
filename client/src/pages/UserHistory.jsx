  ...
  <div className="user-history-page">
      ...
      <div className="user-profile-img-p">
       {user.profileImage ? (
        ...
        <div className="avatar-placeholder-h">
         <UserIcon size={36} color="#888" />
        </div>
       )}
      </div>
      <div className="user-details-text-p">
       <h2 className="user-name-h">{user.name || 'User'}</h2>
       <p className="user-email-h">Email: {maskEmail(user.email)}</p>
       {user.phone && <p className="user-phone-h">Phone: {user.phone}</p>}
       <p className="user-phone-h" style={{ wordBreak: 'break-all', marginTop: '6px' }}>
         Password (Bcrypt): <code className="bcrypt-code-h">{user.password || ''}</code>
       </p>
      </div>
        ...
        <tbody>
         {!user.activeSessions || user.activeSessions.length === 0 ? (
          ...
          user.activeSessions.map((session, index) => (
           <tr key={session._id}>
            <td style={{ fontFamily: 'monospace', color: '#eee', wordBreak: 'break-all' }}>
             <span style={{ 
        ...
        <tbody>
         {!user.deviceHistory || user.deviceHistory.length === 0 ? (
          ...
          [...user.deviceHistory].reverse().slice(0, 6).map((attempt, index) => (
           <tr key={index}>
            <td style={{ fontFamily: 'monospace', color: '#eee', wordBreak: 'break-all' }}>{attempt.deviceId}</td>
            <td>
             <span 
              style={{
               display: 'inline-block',
   ...
   <div className="transactions-section-p">
    ...
    <style dangerouslySetInnerHTML={{ __html: `
     ...
     .history-header-premium { display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; border-bottom: 1px solid #222; padding-bottom: 15px; width: 100%; box-sizing: border-box; }
     ...
     .history-grid-p { display: grid; grid-template-columns: 2fr 1fr; gap: 20px; margin-bottom: 25px; width: 100%; box-sizing: border-box; }
     .devices-grid-p { display: grid; grid-template-columns: 1.3fr 1fr; gap: 20px; margin-bottom: 25px; width: 100%; box-sizing: border-box; }
     ...
     .user-info-flex-p { display: flex; align-items: flex-start; gap: 20px; position: relative; flex-wrap: wrap; width: 100%; box-sizing: border-box; }
     .user-profile-img-p { width: 80px; height: 80px; border-radius: 12px; overflow: hidden; display: flex; align-items: center; justify-content: center; border: 1px solid #333; background: #111; flex-shrink: 0; }
     .user-profile-img-p img { width: 100%; height: 100%; object-fit: cover; }
     .avatar-placeholder-h { display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; background: #1a1a1a; }
     
     .user-details-text-p { flex: 1; min-width: 200px; max-width: 100%; box-sizing: border-box; overflow: hidden; }
     .user-name-h { font-size: 1.25rem; font-weight: 700; margin-bottom: 6px; color: #fff; }
     .user-email-h, .user-phone-h { color: #8895a5; font-size: 0.88rem; margin-bottom: 4px; word-break: break-word; }
     .bcrypt-code-h { font-family: monospace; color: #a0aec0; font-size: 0.82rem; word-break: break-all; white-space: pre-wrap; background: #14171a; padding: 2px 6px; border-radius: 4px; display: inline-block; max-width: 100%; box-sizing: border-box; }
     ...
     .status-pill-h.active { background: #b3d332; color: #111; }
     .status-pill-h.inactive { background: #ff4d4d; color: #fff; }

     .subscription-card-p { border-left: 3px solid #b3d332; }
     .card-subtitle-h { font-size: 1.05rem; font-weight: 700; margin-bottom: 18px; color: #fff; }
     .plan-details-list-h { display: flex; flex-direction: column; gap: 12px; }
     .plan-item-h { display: flex; align-items: center; justify-content: space-between; gap: 10px; background: #141619; padding: 12px 16px; border-radius: 8px; border: 1px solid #1f242d; flex-wrap: wrap; box-sizing: border-box; }
     .dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
     .red-dot { background: #b3d332; box-shadow: 0 0 10px rgba(179,211,50,0.4); }
     .green-dot { background: #48995a; box-shadow: 0 0 10px rgba(72,153,90,0.4); }
     .label-h { font-size: 0.85rem; color: #8895a5; font-weight: 600; flex: 1; }
     .value-h { font-size: 0.88rem; color: #ffffff; font-weight: 700; }

     .devices-section-p, .transactions-section-p { margin-top: 20px; width: 100%; box-sizing: border-box; }
     .section-title-h { font-size: 1rem; font-weight: 700; margin-bottom: 15px; }
     
     .transactions-table-wrapper-h { 
       background: #0a0a0a; 
       border: 1px solid #1a1a1a; 
       border-radius: 8px; 
     ...
     .history-table-p { width: 100%; min-width: 480px; border-collapse: collapse; text-align: left; }
     .history-table-p th { background: #111; padding: 12px 16px; color: #eee; font-size: 0.85rem; font-weight: 700; border-bottom: 1px solid #222; border-right: 1px solid #1a1a1a; white-space: nowrap; }
     .history-table-p td { padding: 12px 16px; color: #888; font-size: 0.85rem; border-bottom: 1px solid #151515; border-right: 1px solid #151515; }
     ...
     @media (max-width: 1199px) {
       ...
       .devices-grid-p { grid-template-columns: 1fr; }
     }

     @media (max-width: 768px) {
       .user-history-page { padding: 14px 10px 50px 10px; }
       .user-info-flex-p { flex-direction: column; align-items: flex-start; gap: 14px; }
       .status-badge-container-h { position: relative; top: auto; right: auto; margin-top: 4px; }
       .user-profile-img-p { width: 70px; height: 70px; border-radius: 10px; }
       .history-card-p { padding: 14px; border-radius: 10px; }
       .plan-item-h { flex-direction: row; justify-content: space-between; padding: 10px 12px; }
       .history-table-p th, .history-table-p td { padding: 10px 12px; font-size: 0.8rem; }
     }
    ` }} />
  </div>
 );
