   ...
   <FrontendLayout isTransparent={false}>
    <div className="fe-viewall-page-v">
     <div className="fe-viewall-header-v">
      <button className="back-btn-v" onClick={() => navigate(-1)} aria-label="Go Back">
       <ChevronLeft size={20} />
      </button>
      <div className="header-info-v">
       <h1 className="header-title-text-v">
        {title ? title.replace(/%20/g, ' ').toUpperCase() : type?.toUpperCase().replace('-', ' ')}
       </h1>
      </div>
     </div>
    ...
    <style dangerouslySetInnerHTML={{ __html: `
     ...
     .fe-viewall-header-v { display: flex; align-items: center; gap: 16px; margin-bottom: 36px; }
     .back-btn-v { 
     ...
     .back-btn-v:hover { 
       ...
       box-shadow: 0 4px 16px rgba(179, 211, 50, 0.4);
     }
     
     .header-info-v { display: flex; align-items: center; }
     .header-title-text-v { 
       color: #b3d332; 
       font-size: 1.35rem; 
       font-weight: 800; 
     ...
     @media (max-width: 768px) {
      .fe-viewall-header-v { gap: 12px; margin-bottom: 22px; }
      .back-btn-v { width: 36px; height: 36px; min-width: 36px; border-radius: 10px; }
      .header-title-text-v { font-size: 1.05rem; letter-spacing: 1px; }
