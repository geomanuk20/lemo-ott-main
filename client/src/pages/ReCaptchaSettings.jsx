...
const ReCaptchaSettings = () => {
 ...
 const [formData, setFormData] = useState({
  siteKey: 'Hidden in Demo',
  secretKey: 'Hidden in Demo',
  login: 'OFF',
  signup: 'OFF',
  forgotPassword: 'OFF',
  contactUs: 'ON'
 ...
 const handleSubmit = async (e) => {
  ...
  try {
   ...
   if (response.ok) {
    showNotification('reCAPTCHA settings saved successfully');
   } else {
 ...
 return (
  <div className="recaptcha-settings-page">
   {notification && (
   ...
   <form onSubmit={handleSubmit} className="settings-form-v">
    <div className="form-content-v">
     ...
     <div className="section-header-v">
      <RefreshCw size={18} className="section-icon-v" />
      <h2 className="section-title-v">reCAPTCHA Settings</h2>
     </div>

     <div className="form-row-full-v">
      <label>Site Key</label>
      <input type="text" name="siteKey" value={formData.siteKey} onChange={handleChange} />
     </div>

     <div className="form-row-full-v">
      <label>Secret Key</label>
      <input type="text" name="secretKey" value={formData.secretKey} onChange={handleChange} />
     </div>

     <div className="section-divider-v"></div>

     <div className="form-row-full-v">
      <label>Display on Login</label>
      <select name="login" value={formData.login} onChange={handleChange}>
     ...
     <div className="form-row-full-v">
      <label>Display on Signup</label>
      <select name="signup" value={formData.signup} onChange={handleChange}>
       <option value="ON">ON</option>
       <option value="OFF">OFF</option>
      </select>
     </div>
     ...
     <div className="form-row-full-v">
      <label>Display on Forgot Password</label>
      <select name="forgotPassword" value={formData.forgotPassword} onChange={handleChange}>
       <option value="ON">ON</option>
       <option value="OFF">OFF</option>
      </select>
     </div>
     ...
     <div className="form-row-full-v">
      <label>Display on Contact Us</label>
      <select name="contactUs" value={formData.contactUs} onChange={handleChange}>
       <option value="ON">ON</option>
       <option value="OFF">OFF</option>
      </select>
     </div>
   ...
   <style dangerouslySetInnerHTML={{ __html: `
    .recaptcha-settings-page { background: #000; min-height: 100vh; padding: 30px 50px; color: #fff; animation: fadeIn 0.4s ease; }
    ...
    .section-header-v { display: flex; align-items: center; gap: 10px; margin-bottom: 25px; }
    .section-icon-v { color: #fff; opacity: 0.8; }
    .section-title-v { font-size: 1rem; font-weight: 800; color: #fff; margin: 0; }
    
    .section-divider-v { height: 1px; background: #111; margin: 25px 0; width: 100%; border-bottom: 1px solid #111; }
