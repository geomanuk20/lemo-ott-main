...
const EditPocketReelSeries = () => {
   ...
   try {
    ...
    if (showData) {
     setFormData({
      ...
      sortInfo: showData.sortInfo || '',
      upcoming: showData.upcoming || 'No',
      seriesAccess: showData.seriesAccess || 'Paid',
      language: showData.language || '',
      genres: showData.genres || [],
      actors: (showData.actors || []).map(a => typeof a === 'object' ? a._id : a),
      directors: (showData.directors || []).map(d => typeof d === 'object' ? d._id : d),
      imdbRating: showData.imdbRating || '',
      contentRating: showData.contentRating || '16+',
 ...
 const handleSubmit = async (e) => {
  ...
  try {
   ...
   if (response.ok) {
    ...
    alert(`Failed to update Pocket Reel Series: ${errorData.message}`);
   }
  } catch (err) {
   console.error('Error updating Pocket Reel Series:', err);
   alert('An error occurred while updating the pocket reel series.');
  } finally {
   setSaving(false);
  }
 };
 ...
 const handleSelectActor = (actorId) => {
  setFormData(prev => {
   ...
   const updated = exists ? prev.actors.filter(aid => aid !== actorId) : [...prev.actors, actorId];
 ...
 const handleSelectDirector = (directorId) => {
  setFormData(prev => {
   ...
   const updated = exists ? prev.directors.filter(did => did !== directorId) : [...prev.directors, directorId];
 ...
 if (loading) {
  return (
   <div className="add-show-page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    <Loader size="small" />
   </div>
  );
 }
 ...
 return (
  <div className="add-show-page">
   <div className="page-header-row">
    ...
    <h2 className="section-title">Edit Pocket Reel Series</h2>
   </div>
   ...
   <form onSubmit={handleSubmit} className="show-form">
    ...
    <div className="form-actions">
     <button type="submit" className="submit-btn" disabled={saving}>
      <Save size={16} />
      <span>{saving ? 'Saving...' : 'Update Pocket Reel Series'}</span>
     </button>
    </div>
   </form>
...
export default EditPocketReelSeries;
