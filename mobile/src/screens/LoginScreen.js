...
export default function LoginScreen({ navigation }) {
  ...
  // Deep Link Listener for Mobile Auth redirection from Browser
  useEffect(() => {
    const handleDeepLink = async (event) => {
      const url = event.url;
      console.log('[LoginScreen] Deep link received URL:', url);
      if (url && (url.startsWith('lemoott://login') || url.includes('token='))) {
        try {
          const match = url.match(/token=([^&]+)/);
          if (match && match[1]) {
            const token = decodeURIComponent(match[1]);
            setLoading(true);
            const result = await socialLoginReal(token, 'Google');
            if (!result.success) {
          ...
          console.error('[LoginScreen] Deep link auth error:', err);
          setErrorMsg('Failed during Google authentication redirect.');
        } finally {
          setLoading(false);
        }
      }
    ...
    // Listen to initial URL (if app was completely closed)
    Linking.getInitialURL().then((url) => {
      if (url) handleDeepLink({ url });
    });

    // Listen to active app deep links
  ...
  const handleSocialLogin = async (provider) => {
    ...
    if (provider === 'Google') {
      ...
      try {
        ...
        console.log('[LoginScreen] Launching system browser for Google login:', authUrl);
        await Linking.openURL(authUrl);
      } catch (err) {
        console.error('[LoginScreen] Failed to open Google login URL:', err);
        setErrorMsg('Failed to launch Google login browser.');
      }
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
          <View style={styles.logoContainer}>
            ...
            <Text style={styles.tagline}>Stream Movies & Web Series Instantly</Text>
          </View>

          <View style={styles.formContainer}>
            <Text style={styles.headerTitle}>Welcome Back</Text>
            <Text style={styles.headerSubtitle}>Sign in to continue watching</Text>
